...
static NSData *aesKey;
static NSData *aesIv;

static NSData *aesKeyNew;
static NSData *aesIvNew;
...
- (id)secretObjectForKey:(NSString *)defaultName
{
    ...
    @try {
       ...
       */
        ...
        if (unarchiver == nil && aesKeyNew != nil) {
            result = [CocoaSecurity aesDecryptWithData:data key:aesKeyNew iv:aesIvNew];
            unarchiver = [[NSKeyedUnarchiver alloc] initForReadingWithData:result.data];
        }
...
{
    CocoaSecurityResult *keyData = [CocoaSecurity sha384:_secret];
    aesKey = [keyData.data subdataWithRange:NSMakeRange(0, 32)];
    aesIv = [keyData.data subdataWithRange:NSMakeRange(32, 16)];
}

- (void)setDataNew
{
    CocoaSecurityResult *keyData = [CocoaSecurity sha384:nil];
    aesKeyNew = [keyData.data subdataWithRange:NSMakeRange(0, 32)];
    aesIvNew = [keyData.data subdataWithRange:NSMakeRange(32, 16)];
}
